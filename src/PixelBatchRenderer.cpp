    ...
    : m_pixelVertices(pixelVertices), m_shader("../res/shaders/pixel.vs", "../res/shaders/pixel.fs")
{
    ...
    glCheck(glBufferData(GL_ARRAY_BUFFER, m_pixelVertices.size() * sizeof(PixelVertex), m_pixelVertices.data(), GL_DYNAMIC_DRAW);)

    glCheck(glEnableVertexAttribArray(0);)
    glCheck(glVertexAttribPointer(0, 2, GL_FLOAT, GL_FALSE, sizeof(PixelVertex), (const void*)offsetof(PixelVertex, position));)

    glCheck(glEnableVertexAttribArray(1);)
    glCheck(glVertexAttribPointer(1, 4, GL_FLOAT, GL_FALSE, sizeof(PixelVertex), (const void*)offsetof(PixelVertex, color));)
...
void PixelBatchRenderer::UpdateVerticesData(const std::vector<PixelVertex>& pixelVertices)
{
